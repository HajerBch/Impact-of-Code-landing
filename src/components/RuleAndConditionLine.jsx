...
const RuleAndConditionLine = ({direction, content, icon}) => {
  return (
    ...
    <p className={'flex-grow roboto  text-xl md:text-2xl  text-right opacity-0'+ (direction === 'left' ? ' text-white opacity-100':' hidden md:block') }>
   {content}
    </p>
    ...
    <p className={'flex-grow roboto text-xl md:text-2xl opacity-0'+ (direction === 'right' ? ' text-white opacity-100':' hidden md:block') }>
    {content}
    </p>
    </div>
  )
